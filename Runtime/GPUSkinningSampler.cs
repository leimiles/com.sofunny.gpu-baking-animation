...
public class GPUSkinningSampler : MonoBehaviour
{
    ...
    [HideInInspector]
    [SerializeField]
    public MilesSkinningShaderType shaderType = MilesSkinningShaderType.GPUSkinning;
    ...
    private void GenerateBonesGUID(GPUSkinningBone[] bones)
    {
        ...
        {
            string boneHierarchyPath = MileSkinningUtils.BoneHierarchyPath(bones, i);
            string guid = MileSkinningUtils.MD5(boneHierarchyPath);
            bones[i].guid = guid;
        }
    }
    ...
    */
    private void SetCurrentAnimationClipV2()
    {
        if (animation == null)
        {
            AnimatorOverrideController animatorOverrideController = new AnimatorOverrideController();
            ...
            for (int i = 0; i < clips.Length; ++i)
            {
                KeyValuePair<AnimationClip, AnimationClip> keyValuePair = new KeyValuePair<AnimationClip, AnimationClip>(clips[i], animClip);
                newOverrides.Add(keyValuePair);
            }
            animatorOverrideController.runtimeAnimatorController = runtimeAnimatorController;
    ...
    {
        Shader shader = Shader.Find("SoFunny/Mini/MiniSkinning");
        /*
        if (createNewShader)
        {
            string shaderTemplate = "GPUSkinning";
